...
// GetVirtualMachineDetailWithMultiTenancy returns the details of a named VirtualMachine from a particular namespace.
func GetVirtualMachineDetailWithMultiTenancy(client kubernetes.Interface, metricClient metricapi.MetricClient, tenant, namespace, name string) (
	*VirtualMachineDetail, error) {
	...
	eventList, err := p.GetEventsForPodWithMultiTenancy(client, dataselect.DefaultDataSelect, pod.Tenant, pod.Namespace, pod.Name)
	nonCriticalErrors, criticalError = errorHandler.AppendError(err, nonCriticalErrors)
	if criticalError != nil {
		return nil, criticalError
	}
	...
	podDetail := toVirtualmachineDetail(pod, metrics, configMapList, secretList, virtualMachineController,
...
func toVirtualmachineDetail(pod *v1.Pod, metrics []metricapi.Metric, configMaps *v1.ConfigMapList, secrets *v1.SecretList,
...
func evalValueFrom(src *v1.EnvVarSource, container *v1.Container, pod *v1.Pod,
	...
	case src.FieldRef != nil:
		...
		valueFrom, err := p.ExtractFieldPathAsString(pod, internalFieldPath)
		if err != nil {
			log.Println(err)
			return ""
		}
